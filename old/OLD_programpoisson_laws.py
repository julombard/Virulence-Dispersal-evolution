...
ReproductionS = classes.Event(name='Reproduction S', propensity='bi*(1- omega * (self.S+self.I)/k) * self.S',
                              Schange='1', Ichange='0', order=1)
DeathS = classes.Event(name='Death S', propensity='di*self.S', Schange='-1', Ichange='0', order=3)
...
Infection = classes.Event(name='Infection', propensity='beta0 *(alpha / (1+alpha) )*self.S*self.I', Schange='-1',
                          Ichange='1', order=2)
...
while sim_time < tmax:
    ...
    if TauPrime < 10 / aox:  # Take 10/aox 1 is left for ignoring this part
        ...
        for i in Propensities_per_site :
            Poisson_means = np.multiply(Tau, np.array(i))
            PoissMeans.append(list(Poisson_means))

        triggers = []
        for i in PoissMeans:
            MeansSites =[]
            for j in i :
                kj = np.random.poisson(j,1)
                MeansSites.append(kj[0])
            triggers.append(MeansSites)
        print('Occurences par sites', triggers, len(triggers))
        ...
        for index, Site in enumerate(ListSites):
            trigger_persite = triggers[index]  # We get the number of times it should trigger during tau
            ...
            for index, event in enumerate(Events):  # For each event
                if 'Dispersal' in event.name:
                    ...
                    nbmigrants = max(abs(trigger_persite[index] * event.Schange),
                                     abs(trigger_persite[index] * event.Ichange))
                    ...
                    for i in range(SuccessfulMigrants):
                        ...
                        if abs(event.Schange) > 0:  # if S are dispersers
                            ...
                            print('ERROR : disperser is neither S nor I and that is very curious !')  # This is useless, the error never raises
                else:
                    # Multiply the state change in population by the number of triggers
...
for index, colname in enumerate(data):
    if index == 0:
        ...
        data[colname] = Densities_out[index - 1]  # I don't remember why i put -1, but it doesn't work if it's removed
print(data)
# Saving into .csv file
