...
for (i in 1:length(listfiles)){
  ...
  for (i in 1:length(list_df_traits)){
    ...
    list_df_traits[[i]]$t <- round(list_df_traits[[i]]$t, 2) #round t values
...
legend(150,41000, legend = c('d = 0.1','d =0.2','d =0.3','d =0.4', 'd =0.5'), col = c(1,2,3,4,5), lty = 1:2, cex = 0.8)
...
for (i in 1:length(TraitsFiles)){
  ...
  LastValues<- subset(ActualDf, ActualDf$t > 170)
...
ConvergeValues
dvalues = c(0.1,0.2,0.3,0.5,0.7, 0.8, 0.9)
plot(dvalues, ConvergeValues, xlab = 'd', ylab='Stable Value', col = 'red', pch =19,ylim = c(0,0.5))
